...
/**
 * @title BlockStreet's BErc20 Contract
...
contract BErc20 is BToken, BErc20Interface {
    ...
    function liquidateBorrow(address borrower, uint repayAmount, BTokenInterface bTokenCollateral) override external returns (uint) {
        liquidateBorrowInternal(borrower, repayAmount, bTokenCollateral);
        return NO_ERROR;
    }
    ...
    function sweepToken(EIP20NonStandardInterface token) override external {
        require(msg.sender == admin, "BErc20::sweepToken: only admin can sweep tokens");
