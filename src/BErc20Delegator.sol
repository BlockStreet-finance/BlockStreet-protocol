...
/**
 * @title BlockStreet's BErc20Delegator Contract
...
contract BErc20Delegator is BTokenInterface, BErc20Interface, BDelegatorInterface {
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function liquidateBorrow(address borrower, uint repayAmount, BTokenInterface bTokenCollateral) override external returns (uint) {
        bytes memory data = delegateToImplementation(abi.encodeWithSignature("liquidateBorrow(address,uint256,address)", borrower, repayAmount, bTokenCollateral));
        return abi.decode(data, (uint));
    }
